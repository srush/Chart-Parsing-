...
instance (Show sig, Show semi) => Show (Chart sig semi)where  
    show (Chart m) = intercalate "\n" $
           map (\((i,j),v) -> printf "[%d:%d] \n\t" i j ++  show v )  $ filter (\(ok, Cell v) -> not $ M.null v) $ M.toList m 
...
type SingleDerivationRule item = item -> Maybe item 
type DoubleDerivationRule item = item -> item -> Maybe item
